...
| パッケージ | 言語 | 位置づけ | 主な利用者 |
|---|---|---|---|
| `gyuma`（Goバイナリ） | Go | CLIツール本体 | エンドユーザー（CLI）、他言語からバイナリ直接呼び出し |
| `gyuma`（npm） | TypeScript | JSラッパー | Node.js CLIツール（Ginueなど）、Node.jsから使いたい人 |
...
**CLIエンドユーザー**  
`gyuma` のGoバイナリを直接使う。Node.js 不要。

**Node.jsのCLIツール開発者（Ginueなど）**  
`gyuma`（npm）を `require()` する。内部でGoバイナリを呼ぶことを意識する必要はなく、`access_token` が返ってくればよい。

### リポジトリ構成

| リポジトリ | 内容 |
|---|---|
| `goqoo-on-kintone/gyuma` | Goバイナリ + npmパッケージ（JSラッパー）を同一リポジトリで管理 |
...
| パッケージ名 | 内容 |
|---|---|
| `gyuma` | JSラッパー（メインパッケージ） |
| `gyuma-darwin-arm64` | Goバイナリ（Apple Silicon） |
| `gyuma-darwin-x64` | Goバイナリ（Intel Mac） |
| `gyuma-linux-x64` | Goバイナリ（Linux x86_64） |
| `gyuma-linux-arm64` | Goバイナリ（Linux ARM64） |
| `gyuma-win32-x64` | Goバイナリ（Windows x64） |
