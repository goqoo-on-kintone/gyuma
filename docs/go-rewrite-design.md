...
> JS/TSのWebサービス向けコアライブラリ（`@goqoo/gyuma-core`）は、具体的なユースケースが生まれた時点で検討する。現時点では作らない（YAGNI）。

### 依存関係

```
gyuma-core（Go）
kintone OAuth 薄いライブラリ
・GenerateAuthURL()
・ExchangeCode()
・RefreshToken()
        ↑
...
- [ ] gyuma-core の将来拡張：state の生成・検証・保存は呼び出し側の責務として割り切る。FaaS等への対応が必要になった際も gyuma-core 自体は変更せず、呼び出し側が StateStore を実装する設計とする
