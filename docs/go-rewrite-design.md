...
現行は `writeToken` でリフレッシュトークンを削除している（保存しない設計）。  
...
```
1. アクセストークンが期限切れ
2. リフレッシュトークンが保存されていれば → トークンエンドポイントに refresh_token グラントで POST
3. 新しいアクセストークンを取得（ブラウザ起動なし）
4. リフレッシュトークンも期限切れ or 存在しない → 通常の認可フロー（ブラウザ起動）
```

---

## クレデンシャルのプレーンテキスト保存モード

デフォルトは暗号化。`--plain` オプションでオプトアウトできる。

```
--plain   クレデンシャルを暗号化せずプレーンテキストで保存する
```

### プレーンテキストモードが有用なケース

- **macOS Keychain など OS の認証情報ストア**と組み合わせる場合（OS 側で保護するためファイル暗号化は不要）
- **CI/CD 環境**（GitHub Actions Secrets など環境変数で渡す運用）
- **開発者が自分のマシンで使う場合**（`~/.config/` はそもそも本人しか読めないパーミッション）

パスワードをスクリプトに書いて本末転倒になるより、プレーンテキスト + ファイルパーミッション制限の方がシンプルかつ安全なケースがある。

### 初回使用時の警告

`--plain` 指定時は stderr に警告を表示し、ファイルパーミッションを自動で 600 に設定する：

```
Warning: credentials will be stored in plain text.
Make sure the file permissions are restricted (chmod 600).
```
